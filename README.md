...
自动模式只在真实用户预授权覆盖当前任务、范围和交互方式，且主控批准当前确切 plan／scope 时继续；非自动模式仍等待用户确认当前确切版本。代码按已确认计划拆分中文本地 commit；默认不 push、不修改远端，方案或审查通过不扩大 Git 与远端操作授权。本仓库仅说明这三个 Skill 的协作边界，未实现 task-workflow，也未据此声明真实 session 试运行已经完成。
