...
可组合的 Codex Skills：日常实现与调试由当前开发 Agent 按项目规范完成；`solution-planner` 按风险负责方案、按持续性负责普通长任务跟踪，`parallel-feature-workflow` 负责并行编排，`local-pr-review` 负责固定范围的独立审查。
...
- 实际派发、新增 Agent 或改变配置时告知分工、模型/推理强度、来源及可修改方式；单 Agent 日常任务不重复播报。所有角色默认继承当前配置，用户偏好通过既有 agent_policy 按单 Agent、角色、任务默认分配；显式配置须按当前派发工具核验，不静默替换。
