...
外层 workflow 交接时，必须传入 plan 或已确认需求／issue 范围、目录、固定且已提交的 base/head、初始工作区状态、无写者证明、模型配置和 `confirmation_basis`。自动模式只有真实用户预授权覆盖当前任务、交互方式与边界，且主控已经批准当前确切 plan/scope（有 plan 时附其确切版本；无 plan 时使用已确认需求／issue 范围）时，二者才可共同写入 `confirmation_basis`；这不等于用户逐版确认。非自动模式仍须保留用户对当前确切范围的确认。依据缺失、版本或范围不一致、超出预授权或存在未决问题时停止交接，不能用“自动”或旧批准代替确认。
...
3. 外层 workflow 交接固定范围时，必须显式传入已解析的 `--base`，并在启动前核对当前已提交 HEAD 与交接 HEAD 完全相同；不得改走 PR 探测或默认分支推断。独立使用本 Skill 时，用户提供目标分支则传入 `--base`；用户未提供时才允许脚本先通过只读 `gh pr view` 获取当前 PR 的精确 base commit，再回退到本地远程默认引用。
...
6. 默认要求工作区干净。发现脏工作区时停止并说明；只有用户明确接受风险时才使用 `--allow-dirty`。workflow 交接还要求整个 Review 期间没有写者，初末工作区状态与固定 HEAD 均不变；任一变化使本轮结果作废。
...
10. 依次报告阻断问题、本次变更但未达到阈值的问题、既有或范围外或不确定的告知项，再分别给出“执行是否完整”和“有无阻断问题”两项结论，说明执行模式、汇总模式和剩余风险；脚本回退模式同时报告文件路径。发现问题不表示执行不完整，执行完整也不表示没有阻断问题。不得自动修复代码或发布评论；修复范围和复审由外层 workflow 主控决定，非自动模式须携带用户选择的修复项。

简略模式下如用户选择 Codex 原生单次 Review，外层 workflow 使用实际已核验的原生入口，并如实报告覆盖；该路由不进入本 Skill，也不得把单次 Review 冒充五视角审查。原生能力不可用时回报缺口，由用户决定后续方式，不能静默降级。选择本 Skill 时仍完整执行以下五视角和独立汇总。
...
8. 汇总完成后重新检查 HEAD 和工作区状态；发生变化时作废结果。修复产生新 HEAD 后，旧结论失效，必须在新固定版本上重新审查；范围未变时可沿用同一 `scope_id`。
...
workflow 交接固定 base 时必须始终追加 `--base "<已核验 base>"`，并由调用方先核对当前 HEAD 与交接的固定 HEAD；不得省略参数而触发 PR 或默认分支探测。脚本参数与报告 schema 保持原样，外层配置仅投影为既有 `--agent-config` 格式，不新增 schema。
