...
- Review 后先按证据分诊 Finding：局部实现错误进入修复；需求／problem framing、诊断根因或 Plan 前提被证据否定时从最上游失效门禁重建。非自动模式由用户选择修复项；自动模式按预授权和问题链证据门禁处理。修复产生新 HEAD 后使用所选方式复审，旧 HEAD 结论不复用。
...
flowchart TD
    ...
    D -->|是| F[主控分诊并核实证据]
    F --> G{分诊入口}
    G -->|证据不足| H[调查；无新依据则等待用户]
    G -->|局部修复| I{已选定或预授权；同链再次修复有新依据}
    G -->|上游失效| J[重建需求、根因或 Plan 并完成确认]
    I -->|否| K[保留证据并停止自动循环]
    I -->|是| L[修复或按重建方案实施]
    J --> L
    L --> M[验证并将新 HEAD 交给 review]
    M --> A
```

非自动模式没有自行启动的修复闭环；用户选定修复项后才执行。主控核实 Finding 的归因、证据及关联需求／验收／Plan 前提；证据不足时仅调查，证据否定时从需求／problem framing、诊断根因或 Plan 中最上游失效层重建。受影响测试、批准适用性和 Review 结果失效但保留历史。用户选择不修的阻断问题仍如实保留，不能把“用户结束任务”说成“质量已通过”。
...
| 自动修复与重规划 | 按问题链的新增诊断证据、策略变化和可判定验证继续；同链没有新依据则停止自动修复并等待用户 |
| 覆盖复核 | 相邻两次完整 Review 在同一范围发现此前遗漏的阻断项时复核共同前提；复核后下一次仍出现挑战该前提的新阻断项则停止自动循环 |
| 阶段最长执行时间 | 按任务在启动时设置；不把 10 分钟探查阈值当成阶段截止时间 |

Plan Review 的“一次修订后复审仍不通过则上报”沿用已确认规则。问题链按场景、期望、实际、证据及关联根因或假设识别；已关闭问题复现、修复引入回归和共享根因关联旧链，独立遗漏建立新链。每轮记录策略、可证伪预期、针对性验证和结果；仅换 session、模型、Agent、写法或 HEAD 不构成新依据。基础 Plan 重做后同链仍失败时，先复核根因和需求，不凭失败本身宣布根因失效。
...
| W-AC6 | 自动 plan、修复与阻断 | 按预授权推进；同链无新增诊断证据时停止，独立新遗漏不消耗旧链，越界时停止 |
...
| W-AC10 | 等待用户与正常长测试 | 不误判卡死，不借切换 session 或模型绕过问题链证据门禁 |
...
| W-AC18 | Finding 分诊与上游失效 | 区分误报、局部实现、Plan、根因和需求歧义；只在有证据时重建最上游失效层并使下游证据失效 |
| W-AC19 | 收敛与覆盖复核 | 同链无进展停止；基础 Plan 重做后仍失败复核根因／需求；连续遗漏触发一次覆盖复核，仍失败交用户 |

试运行至少覆盖：结构推荐等待确认、用户直接指定、简略模式两种 Review、正常多 session 全流程、仅 discuss、仅 review、不 review 的 code、用户确认恢复、独立新 Finding、同链无进展、基础 Plan 重做后仍失败、连续遗漏覆盖复核、启动失败、旧结果迟到、探查及停止未确认。可控的模拟故障可用于失败路径，但简略模式真实执行、两种 Review 路由及多 session 的真实接力和监控验证不可省略；未通过的能力明确标为未就绪。
...
本文保留 discuss 名称和用户原始多 session 四阶段设计；主控统一交接、单目录执行及新增简略模式均来自本轮讨论。临时故障重试参数和尚未指定的推理强度仅作为启动配置建议，不替代用户最终选择。
