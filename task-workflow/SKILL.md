...
主控在阶段启动、结束、阻塞和恢复时向用户简报当前状态，并附可用的任务记录或报告详情入口；需要用户决定时，明确说明待决定的具体内容，不能只报告 `WAITING_USER`。

`SUCCESS` 表示本阶段已达到交接标准且没有阻止当前交接的未决项；`FAILED` 表示执行故障或质量阻断，必须记录类别；`WAITING_USER` 表示规则无法决定且需要用户选择。三者都不表示创建中或运行中。Review 的执行是否完整与是否有阻断问题独立报告，发现质量问题不等于执行故障；Review 不完整是执行故障，拒绝修复或授权不足不能写成质量通过。

已选择 Review 的任务，只有在当前需求与 plan 的必要验证通过、固定 HEAD 的所选 Review 完整，且每项适用阻断项都有以下证据之一时，才可报告质量通过：在当前 HEAD 验证和完整 Review 中关闭、以证据排除为误报，或因已批准修订范围或验收而不再适用。用户明确免 Review 的 code 路径可按其停止点交付实现、验证和本地提交，但必须说明未审查，不能宣称 Review 通过。用户接受遗留风险可以结束执行，但必须保留该风险，不能称质量通过。

## 调度边界

同一任务同一时刻最多一个 phase 执行。phase 收到会影响全局目标、范围、模式或后续流程的用户指令时，通知主控并暂停受影响工作。主控更新基线，判定失效的 plan、代码、测试和 Review 后再继续。

Review 返回后，主控先分诊每条 Finding：核实归因、证据，以及关联的需求／验收项／Plan 前提。证据已否定的前提与尚待调查的问题必须分开记录；仅局部实现错误可局部修复。只有证据证明失效时，才从最上游被否定的门禁重新建立：需求或 problem framing（与 `solution-planner` 的 `contract_status` 衔接）、诊断型任务的根因，或 Plan。受影响下游的批准适用性、测试和 Review 结果随之失效，但历史记录保留；不要求为所有任务另建三套状态。

分诊后按“Review → 分诊 → 局部修复或上游重建 → 验证 → 新 HEAD Review”交接。非自动模式由用户选择修复项；自动模式只在已确认预授权内继续。phase 只提交证据和受影响范围，不自行重设计、决定失效层级或关闭循环；具体的按问题链收敛、覆盖复核和停止条件见 [session-control.md](references/session-control.md)。
