...
实际派发、新增 Agent 或改变配置前，显示稳定 Agent ID、角色、具体任务、依赖/批次、人数与并发上限、模型/推理强度、逐字段配置来源及状态，并提醒“你可以按角色或单个 Agent 修改模型与推理强度；未修改就按展示配置继续”。相同配置已展示时不因方案输出或阶段切换重复播报；恢复先核对实际状态，有变化或无法核实先前展示记录时再展示。
...
按字段解析 `用户单 Agent > 用户角色 > 用户任务默认 > 继承当前`。用户可用自然语言分配，例如 `coder-1=sol/high；reviewer=astra/xhigh`；将名字映射到展示过的 ID，别名与拼写规范化规则沿用规划协议。只指定模型也要校验继承强度是否被目标支持。能力来源必须是当前实际派发工具；多个匹配、未知组合或缺少覆盖能力时先澄清，不能用其他后端列表猜测。

Planner、Coder、Fix 和 Plan Reviewer 默认均继承当前模型与推理强度，不内置固定型号或强度。用户固定角色偏好通过既有 agent_policy 解析并保留逐字段来源，不将角色偏好扩散到全任务 defaults；代码 Review 沿用其 Skill 的相同继承规则。显式目标组合不可用或无法核实时暂停相关派发，不静默替换。验证失败保留证据，不用更换模型绕过权限、范围或验证阻塞。

用户选择 `strict-plan-execution` 时，按规划协议写入角色请求值：Planner / Plan Reviewer / Reviewer / Aggregator=`astra/low`，Coder / Fix=`terra/high`；逐 Agent 和角色覆盖仍优先。派发前必须核验对应精确模型与强度可用，无法核验或不可用时停止该派发，不得降级为其他模型。
