...
主 Agent 可以兼任规划、架构和编排，不为角色名称额外创建 Agent。Coder 数量由独立任务和当前工具容量决定；Review 的内部五视角和汇总者由 `local-pr-review` 拥有，清单可提前展示但不重复派发。给汇总者预留的是后续容量，不与全部 Reviewer 同时启动。

按字段解析 `单 Agent > 角色 > 任务默认 > 继承当前`。用户可用自然语言分配，例如 `coder-1=sol/high；reviewer=astra/xhigh`；将名字映射到展示过的 ID，别名与拼写规范化规则沿用规划协议。只指定模型也要校验继承强度是否被目标支持。能力来源必须是当前实际派发工具；多个匹配、未知组合或缺少覆盖能力时先澄清，不能用其他后端列表猜测。

默认继承；简单低风险独立实现/辅助任务可在未覆盖用户显式字段的前提下透明降级，并记录理由与验证方式。架构、关键实现、最终 Review 默认保持继承。验证失败保留证据并告知，可使用原继承配置重试，不用降级绕过权限或范围阻塞。
